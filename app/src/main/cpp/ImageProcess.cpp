...
jobject createBitmap(JNIEnv *env, Mat srcData, jobject config) {
    ...
    jmethodID createBitmapMid = env->GetStaticMethodID(bmpCls, "craeteBitmap",
...
//        TM_CCOEFF_NORMED
    ...
    Rect rect(maxLoc.x + img_tpl.cols + 10, maxLoc.y - 5,
              img_idCard.cols - (maxLoc.x + img_tpl.cols) - 50, img_tpl.rows + 15);
    //拿二值的号码
